...
class ClientProgram: public five::Program<ClientConfig> {
    int Run(const ClientConfig& config) override {
        ...
        sched.Run([&config, submitter, &data_to_send] {
            ...
            timings.reserve(config.count * config.fibers);
            ...
            for (int thr = 0; thr < config.fibers; ++thr) {
                fibers.emplace_back(tf::Spawn([config, addr, submitter, &timings, thr, &data_to_send, &dead_packets, &the_start, &first_stop, &first_stop_req_count] {
                    ...
                    data += std::to_string(thr);
                    auto data_expected = data + data;
                    LOG_INFO("connecting to", thr);

                    auto start = five::Instant::now();
                    char buf[65'000];
                    for (int i = 0; i < config.count / config.fibers; ++i) {
                        WHEELS_UNUSED(i);
            ...
            std::cout << "{";
            auto avg = std::accumulate(timings.begin(), timings.end(), five::Duration()).nanos() / timings.size();
            LOG_CRIT("min    = {}", timings[0]);
            std::cout << "\"min\": " << timings[0].nanos() << ",";
            LOG_CRIT("max    = {}", timings[timings.size() - 1]);
            std::cout << "\"max\": " << timings[timings.size() - 1].nanos() << ",";
            LOG_CRIT("avg    = {}us", avg / 1000);
            std::cout << "\"avg\": " << avg << ",";
            for (auto quantile : {50., 90., 95., 99., 99.9, 99.99}) {
                auto idx = size_t(timings.size() * (quantile / 100.));
                LOG_CRIT("p{:<5} = {}", quantile, timings[idx]);
                std::cout << "\"p" << quantile << "\": " << timings[idx].nanos() << ",";
            }
            ...
            std::cout << "\"pps\": " << first_stop_req_count / first_stop.seconds_float();
            std::cout << "}";
        });
